...
impl ReadGdtf for Emitter {
    ...
    fn read_any_attribute(data_holder: &mut Self::DataHolder, attr: Attribute<'_>) -> Result<(), Self::Error> {
        match attr.key {
            ...
            b"DominantWaveLength" => data_holder.dominant_wave_length = Some(read::attr_to_f32(attr)),
            b"DiodePart" => {
                match read::attr_to_str(&attr) {
                    "" => {}
...
impl TestReadGdtf for Emitter {
    fn testdatas() -> Vec<(Option<Self::PrimaryKey>, Option<Self>)> {
        vec![
            (Some(Name("MyName".to_string())), Some(Emitter { color: None, dominant_wave_length: None, diode_part: None, measurements: vec![] })),
            (Some(Name("Light Source 6800 High Power".to_string())), Some(Emitter { color: Some(ColorCie { x: 0.312700, y: 0.329000, Y: 100.0 }), dominant_wave_length: Some(0.0), diode_part: None, measurements: Measurement::testdata_vec() })),
            (Some(Name("LED Engine".to_string())), Some(Emitter { color: Some(ColorCie { x: 0.312700, y: 0.329000, Y: 100.0 }), dominant_wave_length: Some(12.000001), diode_part: Some("MyDiodePart".to_string()), measurements: Measurement::testdata_vec() })),
        ]
    }
    ...
    fn testdatas_xml() -> Vec<String> {
        vec![
            ...
            format!(r#"<Emitter Color="0.312700,0.329000,100.000000" DiodePart="" DominantWaveLength="0.000000" Name="Light Source 6800 High Power">{}</Emitter>"#, Measurement::testdata_xml()),
            format!(r#"<Emitter Color="0.312700,0.329000,100.000000" DiodePart="MyDiodePart" DominantWaveLength="12.000001" Name="LED Engine">{}</Emitter>"#, Measurement::testdata_xml())
        ]
    }
