...
impl ReadGdtf for PhysicalDescriptions {
    ...
    fn read_any_child(data_holder: &mut Self::DataHolder, reader: &mut Reader<&[u8]>, event: BytesStart<'_>, has_children: bool) -> Result<(), Self::Error> {
        match event.name() {
            Emitter::PARENT_NODE_NAME => data_holder.emitters = Emitter::read_hash_map_from_event(reader, event, has_children)?,
            Filter::PARENT_NODE_NAME => data_holder.filters = Filter::read_hash_map_from_event(reader, event, has_children)?,
            ColorSpace::NODE_NAME => data_holder.color_space = Some(ColorSpace::read_single_from_event(reader, event, has_children)?.1),
            DmxProfile::PARENT_NODE_NAME => data_holder.dmx_profiles = DmxProfile::read_vec_from_event(reader, event, has_children)?,
            CriGroup::PARENT_NODE_NAME => data_holder.cris = CriGroup::read_vec_from_event(reader, event, has_children)?,
            Connector::PARENT_NODE_NAME => data_holder.connectors = Connector::read_hash_map_from_event(reader, event, has_children)?,
            Properties::NODE_NAME => data_holder.properties = Some(Properties::read_single_from_event(reader, event, has_children)?.1),
            _ => {}
        }
        Ok(())
    }
...
impl TestReadGdtf for PhysicalDescriptions {
    fn testdatas() -> Vec<(Option<Self::PrimaryKey>, Option<Self>)> {
        vec![
            (None, Some(PhysicalDescriptions {
    ...
    fn testdatas_xml() -> Vec<String> {
        let xml = format!(r#"<PhysicalDescriptions>
        <Emitters>
            {}
        </Emitters>
        <Filters>
