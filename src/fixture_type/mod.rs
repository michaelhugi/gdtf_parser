...
impl ReadGdtf for FixtureType {
    ...
    fn read_any_attribute(data_holder: &mut Self::DataHolder, attr: quick_xml::events::attributes::Attribute<'_>) -> Result<(), Self::Error> {
        match attr.key {
            ...
            b"CanHaveChildren" => data_holder.can_have_children = Some(CanHaveChildren::new_from_attr(attr)),
            _ => {}
        }
        Ok(())
    }
    ...
    fn move_data(data_holder: Self::DataHolder) -> Result<Self, Self::Error> {
        Ok(Self {
            name: data_holder.name.ok_or_else(|| Self::attribute_not_found(b"Name"))?,
            short_name: data_holder.short_name.ok_or_else(|| Self::attribute_not_found(b"ShortName"))?,
            long_name: data_holder.long_name.ok_or_else(|| Self::attribute_not_found(b"LongName"))?,
            manufacturer: data_holder.manufacturer.ok_or_else(|| Self::attribute_not_found(b"Manufacturer"))?,
            description: data_holder.description.ok_or_else(|| Self::attribute_not_found(b"Description"))?,
            fixture_type_id: data_holder.fixture_type_id.ok_or_else(|| Self::attribute_not_found(b"FixtureTypeId"))?,
    ...
    fn testdatas() -> Vec<(Option<Self::PrimaryKey>, Option<Self>)> {
        vec![
            (None, Some(Self {
                ...
                fixture_type_id: Guid::new_from_str("807DC00C-18D5-4133-B781-1A003FA988FA").unwrap(),
                thumbnail: Some(Resource::new_from_str("P12 dunkel")),
                ref_ft: Some(Guid::new_from_str("807DC00C-18D5-4133-B781-1A003FA988FB").unwrap()),
...
mod tests {
    ...
    pub fn test_can_have_children_new_from_attr_owned() {
        assert_eq!(CanHaveChildren::No, CanHaveChildren::new_from_attr(testdata::to_attr_owned(b"No")));
        assert_eq!(CanHaveChildren::Yes, CanHaveChildren::new_from_attr(testdata::to_attr_owned(b"Yes")));
        assert_eq!(CanHaveChildren::Yes, CanHaveChildren::new_from_attr(testdata::to_attr_owned(b"Anything else")));
    }

    #[test]
    pub fn test_can_have_children_new_from_attr_borrowed() {
        assert_eq!(CanHaveChildren::No, CanHaveChildren::new_from_attr(testdata::to_attr_borrowed(b"No")));
        assert_eq!(CanHaveChildren::Yes, CanHaveChildren::new_from_attr(testdata::to_attr_borrowed(b"Yes")));
        assert_eq!(CanHaveChildren::Yes, CanHaveChildren::new_from_attr(testdata::to_attr_borrowed(b"Anything else")));
    }
